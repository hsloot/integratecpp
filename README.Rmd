...
However, while `R` provides an [API for `C`](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#The-R-API) and it is possible to
[mix `C` and `C++`](https://isocpp.org/wiki/faq/mixing-c-and-cpp), doing this can pose a higher
burden for those more familiar with `R` and `Rcpp` than `C++` or `C`.
...
Using our wrapper simplifies numerical integration in `Rcpp` considerably:
