...
if (!"testutils.integratecpp" %in% installed.packages()[, "Package"]) {
    if (requireNamespace("cli", quietly = TRUE)) {
        cli::cli_alert_warning("Installation of {.pkg testutils.integratecpp} failed on {.Platform$OS.type}, trying again with argument {.code upgrade = TRUE}")
    } else {
        message(sprintf(
            "Installation of `testutils.integratecpp` failed on %s, trying again with argument `upgrade=TRUE`",
            .Platform$OS.type
        ))
    }
