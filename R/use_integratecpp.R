...
#'
#' @export
use_integratecpp <- function(name = NULL,
                             min_version = NULL) {
    ...
    if (!check_uses_roxygen()) {
        usethis::ui_stop(
            "{usethis::ui_code(who_is_asking())} is designed to work within packages using {usethis::ui_code(\"roxygen2\")}."
        )
    }
    ...
    use_src_directory <- function() {
        if (fs::dir_exists(fs::path(usethis::proj_path(), "src"))) {
            ...
            usethis::ui_todo(
                "Consider adding the following roxygen comment and run {usethis::ui_code(\"devtools::document()\")} to update {usethis::ui_path(\"NAMESPACE\")}."
            )
    ...
    if (check_linkingto_integratecpp() && !check_otherdeps_integratecpp()) {
        usethis::ui_warn(
            "Repeated calls to {usethis::ui_code(who_is_asking())} trigger a bug in {usethis::ui_code(\"usethis::use_package()\")}, see {usethis::ui_path(\"https://github.com/r-lib/usethis#1648\")}."
        )
        check_linkingto_integratecpp_noversion <- function() {
            df_deps <- desc::desc_get_deps(usethis::proj_get())
            NROW(df_deps[df_deps[, "package"] == "integratecpp" &
        ...
        if (check_linkingto_integratecpp_noversion()) {
            usethis::ui_info(
                "Removing {usethis::ui_value(\"integratecpp\")} from {usethis::ui_field(\"LinkingTo\")} field in DESCRIPTION."
            )
            desc::desc_del_dep("integratecpp", type = "LinkingTo")
            usethis::use_package("integratecpp", type = "LinkingTo", min_version = min_version)
        } else {
            usethis::ui_oops(
                "{usethis::ui_code(who_is_asking())} cannot update version of {usethis::ui_value(\"integratecpp\")} from {usethis::ui_field(\"LinkingTo\")} field in DESCRIPTION."
            )
            usethis::ui_todo(
                "Consider manually updating version of {usethis::ui_value(\"integratecpp\")} from {usethis::ui_field(\"LinkingTo\")} field in DESCRIPTION."
            )
        }
    ...
    use_cpp_standard_11 <- function() {
        ...
        if (!isTRUE(requirements == new_requirements)) {
            usethis::ui_done(
                "Adding {usethis::ui_value(\"C++11\")} to {usethis::ui_field('SystemRequirements')} field in DESCRIPTION"
            )
